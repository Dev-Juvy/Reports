...
DROP PROCEDURE IF EXISTS `HOgetExpressAPISummary`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetExpressAPISummary`(IN db VARCHAR(100),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15))
BEGIN
...
COUNT(bp.kptn) AS txncount,SUM(bp.amount) AS principal,SUM(bp.charge) AS charge,sum(commission) as commission
FROM `KPPartnersExpress`.`apihistory` bp
Where date(txndate) = ''',_datefrom,'''
