...
DROP PROCEDURE IF EXISTS `HOgetExpressEloadSummary`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetExpressEloadSummary`(IN db VARCHAR(100),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15))
BEGIN
...
COUNT(bp.kptn) AS txncount,SUM(bp.amount) AS principal,'''' AS charge,sum(commission) as commission
FROM `KPPartnersExpress`.`eloadhistory` bp
