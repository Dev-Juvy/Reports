...
DROP PROCEDURE IF EXISTS `HOgetWalletLoadingSummary`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetWalletLoadingSummary`(IN db VARCHAR(100),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15))
BEGIN
...
'SELECT 
'''' as walletno,'''' as username, custid, sendername as customername,  principal,
IF(servicecharge IS NULL,0,servicecharge) as charge,1 as txncount
FROM `kpdomestic`.payout',_monthdate,' p
WHERE kptnno LIKE ''%mlw%'' and controlno like ''%lp%'' and year(p.claimeddate)=',_year,' 
#group by custid
');						
