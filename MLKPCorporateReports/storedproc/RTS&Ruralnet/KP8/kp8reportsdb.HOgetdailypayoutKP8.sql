...
DROP PROCEDURE IF EXISTS `HOgetdailypayoutKP8`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetdailypayoutKP8`(IN accountCode VARCHAR(35),IN _year VARCHAR(6),IN _monthday VARCHAR(5),IN _username VARCHAR(50),IN _role VARCHAR(50),IN _db VARCHAR(100),IN _db1 VARCHAR(100),IN zcode VARCHAR(5),IN rcode VARCHAR(5),IN acode VARCHAR(5),IN bcode VARCHAR(5),IN _usertype VARCHAR(10),IN oldzcode VARCHAR(5))
BEGIN
IF _usertype = "NOTIAD" THEN #NOT IAD USER
	...
	SELECT controlno, kptn, referenceno, sendername, receivername, DATE_FORMAT(transdate,''%Y-%m-%d %r'') AS transdate, cancelleddate,
		...
		FROM(
		SELECT DISTINCT 
			IF(oldkptnno IS NULL,p.kptn,oldkptnno) AS kptn, 
			...
			IF(oldkptnno IS NULL,p.kptn,oldkptnno) AS referenceno, 
		...
		FROM kpdomestic2018.payout',_monthday,'  p 
		...
		SELECT DISTINCT 
			...
			receivercontactno AS Receiver_Phone, s.kptn AS referenceno,s.Currency, s.principal,0 AS servicecharge,charge, 
		...
		FROM  kpdomestic2018.sendout',_monthday,' s
...
ELSEIF _usertype = "IAD" THEN #IAD USER
	IF accountCode = "" THEN #NO SPECIFIC PARTNER - FOR SUMMARY REPORT
		...
		SELECT controlno,kptn,referenceno,sendername,receivername,DATE_FORMAT(transdate,''%Y-%m-%d %r'') AS transdate,cancelleddate,
			...
			SELECT DISTINCT 
				...
				s.oldkptnno AS oldkptn,receivercontactno AS Receiver_Phone,s.kptn AS referenceno,s.Currency,
	...
	ELSEIF accountCode <> "" THEN #WITH SPECIFIC PARTNER 
		...
		SELECT controlno,kptn,referenceno,sendername,receivername,DATE_FORMAT(transdate,''%Y-%m-%d %r'') AS transdate,
			...
			SELECT DISTINCT 
				...
				SELECT DISTINCT 
					...
					DATE_FORMAT(s.transdate,''%H:%i:%S'') AS TIME,s.transdate AS sodate,
					CONCAT(s.receiverlname,'', '',s.receiverfname,'' '',s.receivermname) AS receivername,
					s.oldkptnno AS oldkptn,receivercontactno AS Receiver_Phone, s.kptn AS referenceno,
		...
		SELECT controlno,kptn,referenceno,sendername,receivername,DATE_FORMAT(transdate,''%Y-%m-%d %r'') AS transdate,cancelleddate,
			...
			SELECT DISTINCT 
				s.kptn,IF(s.cancelreason=''RETURN TO SENDER'',''*'','''') AS flag, 
				s.cancelleddate AS cancelleddate,cancelreason, s.controlno,
				CONCAT(s.senderlname,'', '',s.senderfname,'' '',s.sendermname) AS sendername,transdate,
				DATE_FORMAT(s.transdate,''%H:%i:%S'') AS TIME,s.transdate AS sodate,
				CONCAT(s.receiverlname,'', '',s.receiverfname,'' '',s.receivermname) AS receivername,
		...
		SELECT controlno,kptn,referenceno,sendername,receivername,DATE_FORMAT(transdate,''%Y-%m-%d %r'') AS transdate,
			...
			SELECT DISTINCT 
				...
				s.kptn AS referenceno,s.Currency, s.principal,0 AS servicecharge,charge,
