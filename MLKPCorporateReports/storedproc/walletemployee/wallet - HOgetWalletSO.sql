...
DROP PROCEDURE IF EXISTS `HOgetWalletSO`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetWalletSO`(IN accountCode VARCHAR(30),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15),IN prefix VARCHAR(5))
BEGIN
...
'''') AS flag,
s.transdate,s.kptnno AS kptn,receivername,sendername,'''' AS sobranch,'''' AS pobranch,'''' AS partnername,CONVERT(AES_DECRYPT(principal,''mlinc1234''), DECIMAL(11,2)) as principal,
...
and if((receivername<>sendername and AES_DECRYPT(charge,''mlinc1234'')=(select charge from `kpforms`.`wallettowalletFixCharge` limit 1) ),1,0) 
...
username,fullname,operator,cancelreason,if(cancelleddate is null,''0000-00-00 00:00:00'',cancelleddate) as cancelleddate,oldkptn,cancelledbyoperator,custid,controlno,branchcode,zonecode,recvrwalletid 
