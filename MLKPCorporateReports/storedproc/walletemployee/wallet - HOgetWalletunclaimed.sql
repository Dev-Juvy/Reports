...
DROP PROCEDURE IF EXISTS `HOgetWalletUnclaimed`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetWalletUnclaimed`(IN accountCode VARCHAR(30),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15),IN prefix VARCHAR(5))
BEGIN
...
'SELECT '''' AS flag,
s.transdate,s.kptnno AS kptn,'''' as receivername,sendername,'''' AS sobranch,'''' AS pobranch,'''' AS partnername,
...
and year(s.transdate)=',_year,' and cancelleddate is null and cancelreason is null and IsClaimed = 0 order by transdate asc;
');													
