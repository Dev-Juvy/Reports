...
DROP PROCEDURE IF EXISTS `HOgetWalletPaymentSO`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `HOgetWalletPaymentSO`(IN accountCode VARCHAR(30),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15),IN prefix VARCHAR(5))
BEGIN
...
and year(s.transdate)=',_year,'  and kptnno like ''%mlp%'' and if(''',accountCode,'''='''',1,s.operatorid=''',accountCode,''')
