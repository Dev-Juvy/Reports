...
CREATE DEFINER=`root`@`%` PROCEDURE `HOgetWalletLoadingSummaryEmp`(IN db VARCHAR(100),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15))
BEGIN
...
WHERE kptnno LIKE ''%mlb%'' and controlno like ''%lp%'' and year(p.claimeddate)=',_year,' 
group by custid
');						
