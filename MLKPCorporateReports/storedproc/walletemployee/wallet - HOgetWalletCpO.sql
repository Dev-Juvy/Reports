...
DROP PROCEDURE IF EXISTS `HOgetWalletCPO`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetWalletCPO`(IN accountCode VARCHAR(30),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15),IN prefix VARCHAR(5))
BEGIN
...
'select flag,if(transdate is null,''0000-00-00 00:00:00'',transdate) as transdate,kptn,receivername,sendername,sobranch,pobranch,
partnername,principal,charge,commission,accountid,
username,fullname,operator,cancelreason,if(cancelleddate is null,''0000-00-00 00:00:00'',cancelleddate) as cancelleddate,
oldkptn,cancelledbyoperator,custid,controlno,branchcode,zonecode 
from (SELECT '''' AS flag, 
s.claimeddate as transdate,if(s.oldkptnno is not null,s.oldkptnno,s.kptnno) AS kptn,receivername,sendername,'''' AS sobranch,'''' AS pobranch,'''' AS partnername,
CONVERT(AES_DECRYPT(principal,''mlinc1234''), DECIMAL(11,2)) as principal,
CONVERT(AES_DECRYPT(cancelledcustcharge,''mlinc1234''), DECIMAL(11,2)) as charge,
0 AS commission,
...
and year(s.claimeddate)=',_year,' AND cancelledreason=''Wrong Payout''
order by claimeddate asc)x;
');													
