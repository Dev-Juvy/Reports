...
DROP PROCEDURE IF EXISTS `HOgetWalletBPRFCEmp`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetWalletBPRFCEmp`(IN accountCode VARCHAR(30),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15))
BEGIN
...
WHERE year(s.cancelleddate)=',_year,' AND  kptnno LIKE ''%bpw%''  AND cancelreason=''Change Details''
