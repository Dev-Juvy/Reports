...
DROP PROCEDURE IF EXISTS `HOgetWalletBPCSEmp`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetWalletBPCSEmp`(IN accountCode VARCHAR(30),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15),IN prefix VARCHAR(5))
BEGIN
...
WHERE year(s.cancelleddate)=',_year,' AND  kptnno LIKE ''',prefix,'''  AND (cancelreason=''Cancel'' or cancelreason=''Cancel Sendout'')
