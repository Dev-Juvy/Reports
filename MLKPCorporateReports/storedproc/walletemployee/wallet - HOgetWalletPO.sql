...
CREATE DEFINER=`root`@`localhost` PROCEDURE `HOgetWalletPO`(IN accountCode VARCHAR(30),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15),IN prefix VARCHAR(5))
BEGIN
...
'select flag,if(claimeddate is null,''0000-00-00 00:00:00'',transdate) as claimeddate,
if(transdate is null,''0000-00-00 00:00:00'',transdate) as transdate,kptn,receivername,sendername,sobranch,pobranch,partnername,principal,
...
branchcode,zonecode,recvrwalletid from(
SELECT 
...
claimeddate ,sodate AS transdate,if(s.cancelledreason=''Wrong Payout'',s.oldkptnno,s.kptnno) AS kptn,receivername,sendername,'''' AS sobranch,'''' AS pobranch,'''' AS partnername,
...
SET @sql3=CONCAT('select flag,if(claimeddate is null,''0000-00-00 00:00:00'',transdate) as claimeddate,
if(transdate is null,''0000-00-00 00:00:00'',transdate) as transdate,kptn,receivername,sendername,sobranch,pobranch,partnername,principal,
if(charge is null,0,charge) as charge,commission,accountid,username,fullname,operator,cancelreason,
if(cancelleddate is null,''0000-00-00 00:00:00'',cancelleddate) as cancelleddate,oldkptn,cancelledbyoperator,custid,controlno,
