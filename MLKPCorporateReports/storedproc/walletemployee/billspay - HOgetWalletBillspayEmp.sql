...
DROP PROCEDURE IF EXISTS `HOgetWalletBillspayEmp`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetWalletBillspayEmp`(IN accountCode VARCHAR(30),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15))
BEGIN
...
WHERE s.kptnno LIKE ''%bpm%''  and year(s.transdate)=',_year,'
