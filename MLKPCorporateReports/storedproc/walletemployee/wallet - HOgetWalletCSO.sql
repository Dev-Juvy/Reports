...
DROP PROCEDURE IF EXISTS `HOgetWalletCSO`$$

CREATE DEFINER=`root`@`%` PROCEDURE `HOgetWalletCSO`(IN accountCode VARCHAR(30),IN _datefrom VARCHAR(15),IN _dateto VARCHAR(15), IN prefix VARCHAR(5))
BEGIN
...
'SELECT '''' AS flag, 
s.transdate,s.kptnno AS kptn,receivername,sendername,'''' AS sobranch,'''' AS pobranch,'''' AS partnername,
...
and year(s.cancelleddate)=',_year,' AND cancelreason=''Cancel Sendout''
